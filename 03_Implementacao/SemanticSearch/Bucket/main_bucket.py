...
def api_info():
    ...
    logger.info("Recebido pedido para endpoint de informa��o da API")
    return jsonify({
...
def startup():
    ...
    try:
        ...
        return jsonify({"error": f"inicializa��o falhou: {str(e)}"}), 500
...
if __name__ == "__main__":
    ...
    try:
        bucket_service = BucketWebService()
        logger.info("BucketWebService inicializado, a configurar aplica��o Flask")
        bucket_service.set_app(app)
        logger.info("A iniciar aplica��o Flask")
        app.run(host="0.0.0.0", port=5000, debug=False)
        
    except Exception as e:
        logger.error(f"Falha ao iniciar aplica��o Flask: {str(e)}")
        raise