    ...
    def register_routes(self):
        ...
        async def startup():
            ...
            async with httpx.AsyncClient() as client:
                buckets = self.ai_node.get_buckets()
                if not buckets:
                    return jsonify({'message': 'Nenhum bucket registrado'}), 200
                all_processed = True
        ...
        def register_bucket():
            # Registra bucket no nó AI
            data = request.get_json()
            bucket_name = data.get('bucket_name')
            bucket_url = data.get('bucket_url')
            if bucket_name and bucket_url:
                if self.ai_node.register_bucket(bucket_name, bucket_url):
                    return jsonify({'status': 'success', 'message': f'Bucket {bucket_name} registrado'}), 200
                return jsonify({'status': 'error', 'message': 'Falha no registro'}), 500
