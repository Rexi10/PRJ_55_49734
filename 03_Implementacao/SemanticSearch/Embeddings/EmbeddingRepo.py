...
class EmbeddingRepo:
    def __init__(self):
        ...
        self.faiss_index = faiss.IndexFlatIP(768)  # Inner Product para cosine similarity
    ...
    def search(self, query_vector: np.ndarray, k: int = 3) -> List[Tuple[Doc, float]]:
        ...
        # pede um maior K para lidar com duplicados
        search_k = min(k * 2, self.faiss_index.ntotal)  # Evita solicitar mais do que o disponível
        distances, indices = self.faiss_index.search(np.array([normalized_query]), search_k)
        similarities = distances[0] 
        results = []
        seen_docs = set()
        for j, i in enumerate(indices[0]):
            if len(results) >= k:  # para quando chegarmos a k resultados
                break
            doc, _ = self.docs[i]
